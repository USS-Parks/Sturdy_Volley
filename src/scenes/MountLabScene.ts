...
export class MountLabScene extends GameScene {
  ...
  build(): Scene {
    ...
    this.buildHorseGraybox(scene, this.horseRoot);
  ...
  // --- Graybox geometry (§0.9) ----------------------------------------------

  /**
   * Representative low-poly horse: faceted body + neck + head + four legs + a
   * saddle marker at the mount anchor. Parented to `root` (placed at the feet)
   * so a future `.glb` is a one-line swap of this helper. Withers ≈ 1.6 m.
   */
  private buildHorseGraybox(scene: Scene, root: TransformNode): void {
    const coat = PALETTE.wood; // warm bay/dun brown
    const part = (name: string, opts: { w: number; h: number; d: number }, pos: Vector3, color = coat, emissive = 0.28): void => {
      const m = MeshBuilder.CreateBox(`mount-horse-${name}`, { width: opts.w, height: opts.h, depth: opts.d }, scene);
      m.parent = root;
      m.position.copyFrom(pos);
      m.material = flatMaterial(scene, `mount-horse-${name}`, color, emissive);
      m.isPickable = false;
      this.horseMeshes.push(m);
    };
    ...
    const anchor = this.family.mountAnchor ?? { x: 0, y: 1.5, z: 0 };
